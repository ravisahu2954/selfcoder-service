...
public class WebSecurity {
	...
	//	UserDetails admin = User.withUsername("ravi").password(encoder.encode("python123")).roles("ADMIN").build();
	//	UserDetails user = User.withUsername("raj").password(encoder.encode("python123")).roles("USER").build();
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		return http.csrf().disable()
				...
				.requestMatchers("/selfcoder/v1/users")
			    .permitAll()
			    .and()
			    .authorizeHttpRequests()
			    .requestMatchers("/selfcoder/v1/courses")
			    .permitAll()
			    .and()
			    .authorizeHttpRequests()
			    .requestMatchers("/selfcoder/v1/blogs")
			    .permitAll()
			    .and()
				.authorizeHttpRequests()
				.requestMatchers("/selfcoder/v1/problems")
				.permitAll()
				.and()
		        .build();
     }
